<x-app-layout>
    <x-slot name="title">{{ $news->title }} - Portal Berita</x-slot>
    ...
    <article class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav class="mb-8">
            <ol class="flex items-center space-x-2 text-sm text-gray-500">
                ...
                @if($news->category)
                    <li>/</li>
                    <li><a href="{{ route('news.category', $news->category->slug) }}" class="hover:text-blue-600">{{ $news->category->name }}</a></li>
                @endif
                <li>/</li>
                <li class="text-gray-900 truncate">{{ $news->title }}</li>
            </ol>
        </nav>
        ...
        <header class="mb-8">
            ...
            <!-- Meta Information -->
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between py-4 border-y border-gray-200">
                ...
                <div class="text-sm text-gray-500">
                    <time>
                        Dipublikasikan: {{ $news->published_at ? $news->published_at->format('d M Y, H:i') : $news->created_at->format('d M Y, H:i') }}
                    </time>
                </div>
            </div>
        </header>
